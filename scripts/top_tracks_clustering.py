...
def prime_general_user():
    ...
    desired_user = SpotifyUser('1232063482', contacter=desired_user_contacter, optional_display_id = 'Top Tracks') #my id stays to add to playlists
    return desired_user
...
def main():
    logging.basicConfig(filename='../logs/top_tracks_upload.log', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s', filemode='w')
    og_audio_features = json.load(open('../data/Top Tracks Oct 17 2021_tracks.json'))
    ...
    kmeans_labels, agglomerative_labels = execute_clusterings(kmeans_number=9, agglomerative_number= 9, normalized_data = normalized_data)
